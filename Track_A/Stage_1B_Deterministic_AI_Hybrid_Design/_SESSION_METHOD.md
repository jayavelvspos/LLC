# How to run every session (the loop) — Stage 1B

Use this same 8-step loop for **every** session in **Stage 1B — Deterministic
AI & Hybrid Design** (and every other stage). It fits in ~45 minutes *when no
...
This stage is mostly **judgment and design**, not math — deciding where a
deterministic rule beats a model call. The Track_B footing is light and never
blocking:

| This stage's concept | Track_B topic | Blocking? |
|---|---|---|
| "Is this worth a model call?" as an expected-cost decision | `05_decision_and_orchestration_math` (expected value) | non-blocking — note and revisit |
| Regex, heuristics, scoring functions | — | **no link** — pure engineering |
