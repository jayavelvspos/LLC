# How to run every session (the loop) — Stage 5B

Use this same 8-step loop for **every** session in **Stage 5B — Orchestration,
Frameworks & MCP** (and every other stage). It fits in ~45 minutes *when no
Track_B detour is needed*. Keep a timer — the boxes exist to stop you
...
20–30 min timebox and the session resumes after.
...
switch now. Track_A leads; don't switch on a whim.
...
This stage is mostly **framework and protocol engineering** — LCEL wiring,
tracing, MCP client/server plumbing, low-code tools. The Track_B footing is
light and never blocking:

| This stage's concept | Track_B topic | Blocking? |
|---|---|---|
| Routing & parallelism (branch selection, fan-out/fan-in) | `05_decision_and_orchestration_math` (expected-value routing, DAG scheduling) | non-blocking — note and revisit |
| Evaluator–optimizer convergence (when does the loop stop?) | `05_decision_and_orchestration_math` (absorbing states) + statistics (measuring "better") | non-blocking |
| Reading LangSmith traces / latency percentiles | `Track_B/Math_stat` statistics (p50/p95, distributions) | non-blocking |
| LCEL, MCP hosts/servers, n8n, Langflow | — | **no link** — pure engineering |

If a session says "no Track_B link", write that in `notes.md` and move on — a
valid, common outcome here.
